...
nodes = {
    # Name     CPU, RAM, IP,  BOX
    'm01'  => [  4,   4, MASTERIP, "kube-#{K8S_VERSION}" ],
    'w01'  => [  4,   4, 201, "kube-#{K8S_VERSION}" ],
    'w02'  => [  4,   4, 202, "kube-#{K8S_VERSION}" ]
}
...
Vagrant.configure("2") do |config|
    ...
    nodes.each do |name, (cpu, ram, net, box)|
        ...
        config.vm.box = "kube-#{K8S_VERSION}"
            ...
            box.vm.provision "shell", env: {"SUBNET" => "#{SUBNET}", "IP" => "#{net}" }, inline: <<-SHELL
                ...
                case $HOSTNAME in
	                m01.local)
                        ...
                        sudo cat /etc/kubernetes/admin.conf > ~vagrant/.kube/config
                        chown -R vagrant:vagrant ~vagrant/.kube
						
                        # activate flannel, new shell to read values
